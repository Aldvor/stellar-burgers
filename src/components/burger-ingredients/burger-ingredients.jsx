...
const BurgerIngredients = ({data, addIngredientToCart}) => {
    ...
    const refs = {
        'Булки': React.useRef(null),
        'Соусы': React.useRef(null),
        'Начинки': React.useRef(null),
    };
    ...
    return (
        <section className={styles.section}>
            <h1 className="text text_type_main-large mb-5">Соберите свой бургер</h1>
            ...
            <ul className={`${styles['create-burger']} custom-scroll`}>
                {Object.entries(categorizedItems).map(([category, ingredients]) => (
                    <li key={category} ref={refs[category]}>
                        ...
                        <ul className={`${styles.ingredients} mt-6 pl-4`}>
                            {ingredients.map(({_id, type, name, price, image}) => (
                                ...
                                >
                                    ...
                                    <img className='pl-4 pr-4' src={image} alt={name} width="240" height="120"/>
